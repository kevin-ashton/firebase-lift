...
export const defaultEmptyTask: BatchTaskEmpty = {
  id: '',
  collection: 'empty',
  type: 'empty'
};
...
export function generateQueryRef<ItemModel>(
  ...
  if (queryRequest.where) {
    for (let i = 0; i < queryRequest.where.length; i++) {
      ...
      if (!(r1.value instanceof Array)) {
        throw new Error(
          "Query value must be an array. Example: { where: [{id: ['==', '123']}] }.  Instead encountered the following query: " +
  ...
  if (queryRequest.orderBy) {
    for (let i = 0; i < queryRequest.orderBy.length; i++) {
      let orderBy = queryRequest.orderBy[i];
      let path = generateFirestorePathFromObject(orderBy.pathObj).path;
      query = query.orderBy(path as any, orderBy.dir || undefined) as any;
    }
  }
